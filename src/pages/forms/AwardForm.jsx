...
export default function AwardForm() {
  ...
  const visible = state.visibility.award;
  ...
  const handledelte = (index) => {
    dispatch({ type: "DELETE_AWARD", index });
  };
  ...
  const handlereset = () => {
    dispatch({ type: "RESET_AWARD" });
  };

  const handlevisibilty = () => {
    console.log("cllik visiviityyy");

    dispatch({ type: "VISIBILITY", section: "award" });
  };

  useEffect(() => {
    console.log("crtifacte state updated:", state.award);
  }, [state.award]);
  ...
  return (
    <section className="flex flex-col gap-3  p-3   transition-opacity duration-200 ">
      <div className="formheading flex justify-between items-center">
        <div className="heading font-semibold text-2xl">AWARD 🏆</div>
      ...
      {visible && (
        <>
          {state.award.map((cert, index) => {
            
            const { award,awarddescription   } = cert;
            return (
